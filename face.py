...
# OpenCV 人脸检测
face_patterns_people = cv2.CascadeClassifier('haarcascade_frontalface_default.xml')
face_patterns_dm = cv2.CascadeClassifier('lbpcascade_animeface.xml')

sample_image = cv2.imread('img/red.jpg')
cv2.imshow("after", sample_image)
faces_people = face_patterns_people.detectMultiScale(sample_image, scaleFactor=1.2, minNeighbors=1)
#faces_anime = face_patterns_dm.detectMultiScale(sample_image, scaleFactor=1.15, minNeighbors=1)
